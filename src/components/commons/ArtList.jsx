...
export function ArtList() {
    ...
    useEffect(() => {
        ...
        api.get("/productos?size=5").then((response) => {
    ...
    const artItems = listadoProds.map(function (prod) {
        return (
            <ArtItem key={prod.id_producto} id={prod.id_producto} codigo={prod.codigo} imagen={prod.imagen} alt={prod.imagen_original} />);/*Aca a cada item de ArtItem le pasaba el objeto entero, no es necesario, solo se pasa id, codigo e imagen*/
    });
