...
export function ArtItemDetail() {
  ...
  const imagen = (
    <img
      src={"http://localhost:4000/uploads/" + ruta}
      alt={imagen_original}
      height="50px"
      width="50px"
    ></img>
  ...
  return (
    <div className="articulos">
      <article className="articulo">
        {imagen}
        <h2>Codigo: {codigo}</h2>
        <Link to="#" className="agregarCarroBtn">
          Agregar al Carro
        </Link>
      </article>
      <article className="detalle">
        <ul>
          ...
          {isAdmin ? (
            <li>
              <Link to={"/producto/update/" + id}>
                <img src={btn_edicion} alt="Editar" />
              </Link>{" "}
              <button onClick={handleButtonDel}>
                <img src={btn_borrar} alt="Borrar" />
              </button>
            </li>
          ) : (
            <li></li>
